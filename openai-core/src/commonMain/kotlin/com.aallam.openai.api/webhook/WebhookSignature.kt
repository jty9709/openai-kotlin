package com.aallam.openai.api.webhook

import kotlinx.serialization.SerialName
import kotlinx.serialization.Serializable
import kotlinx.serialization.json.Json
import kotlinx.serialization.json.JsonObject
import kotlin.io.encoding.Base64
import kotlin.io.encoding.ExperimentalEncodingApi

/**
 * Verifies the signature of an incoming webhook request.
 *
 * OpenAI signs webhooks using the `Standard Webhooks` scheme: the signed content is
 * `{webhookId}.{webhookTimestamp}.{body}`, and the signature is the Base64-encoded HMAC-SHA256 of that
 * content keyed by the decoded signing secret. The `webhook-signature` header may carry several
 * space-separated candidates, each optionally prefixed with a version such as `v1,`.
 *
 * This performs the signature check only. Pair it with [isWithinTolerance] to also reject replays of
 * stale requests.
 *
 * @param webhookId the value of the `webhook-id` header.
 * @param webhookTimestamp the value of the `webhook-timestamp` header.
 * @param body the raw request body, exactly as received.
 * @param signatureHeader the value of the `webhook-signature` header.
 * @param secret the endpoint signing secret, optionally prefixed with `whsec_`.
 */
@OptIn(ExperimentalEncodingApi::class)
public fun verifySignature(
    webhookId: String,
    webhookTimestamp: String,
    body: String,
    signatureHeader: String,
    secret: String,
): Boolean {
    val key = if (secret.startsWith(SECRET_PREFIX)) {
        try {
            Base64.decode(secret.removePrefix(SECRET_PREFIX))
        } catch (_: IllegalArgumentException) {
            return false
        }
    } else {
        secret.encodeToByteArray()
    }
    val signedContent = "$webhookId.$webhookTimestamp.$body"
    val expected = Base64.encode(hmacSha256(key, signedContent.encodeToByteArray())).encodeToByteArray()

    return signatureHeader
        .split(Regex("\\s+"))
        .filter { it.isNotBlank() }
        .any { candidate -> constantTimeEquals(expected, candidate.removePrefix("v1,").encodeToByteArray()) }
}

private fun constantTimeEquals(expected: ByteArray, actual: ByteArray): Boolean {
    if (expected.size != actual.size) return false
    var difference = 0
    for (index in expected.indices) {
        difference = difference or (expected[index].toInt() xor actual[index].toInt())
    }
    return difference == 0
}

/**
 * Returns whether [webhookTimestamp] is within [toleranceSeconds] of [nowSeconds].
 *
 * Use this together with [verifySignature] to reject replayed webhook deliveries. The caller
 * supplies [nowSeconds] so that the check stays portable across every target.
 *
 * @param webhookTimestamp the value of the `webhook-timestamp` header.
 * @param toleranceSeconds the maximum accepted age of the timestamp, in seconds.
 * @param nowSeconds the current Unix timestamp in seconds.
 */
public fun isWithinTolerance(
    webhookTimestamp: String,
    toleranceSeconds: Long,
    nowSeconds: Long,
): Boolean {
    val timestamp = webhookTimestamp.toLongOrNull() ?: return false
    val age = if (nowSeconds > timestamp) nowSeconds - timestamp else timestamp - nowSeconds
    return age <= toleranceSeconds
}

/**
 * A webhook delivery.
 *
 * @property id the identifier of the delivery.
 * @property createdAt the Unix timestamp (in seconds) of when the event was created.
 * @property type the event type, e.g. `response.completed`.
 * @property data the payload of the event.
 */
@Serializable
public data class WebhookEvent(
    @SerialName("id") public val id: String? = null,
    @SerialName("created_at") public val createdAt: Long? = null,
    @SerialName("type") public val type: String? = null,
    @SerialName("data") public val data: JsonObject? = null,
)

private val webhookJson = Json { ignoreUnknownKeys = true }

/**
 * Parses a webhook payload into a [WebhookEvent].
 *
 * The payload is kept as a [JsonObject] so that newly introduced event types and fields remain
 * accessible without a client release. Throws if [body] is not valid JSON.
 *
 * @param body the raw request body, exactly as received.
 */
public fun unwrapWebhookEvent(body: String): WebhookEvent =
    webhookJson.decodeFromString(WebhookEvent.serializer(), body)

/**
 * The default tolerance applied to webhook timestamps, in seconds.
 */
public const val DEFAULT_TOLERANCE_SECONDS: Long = 300

private const val SECRET_PREFIX = "whsec_"

private val HEX_DIGITS = "0123456789abcdef".toCharArray()

internal fun ByteArray.toHex(): String {
    val result = CharArray(size * 2)
    for (i in indices) {
        val value = this[i].toInt() and 0xFF
        result[i * 2] = HEX_DIGITS[value ushr 4]
        result[i * 2 + 1] = HEX_DIGITS[value and 0x0F]
    }
    return result.concatToString()
}

// region HMAC-SHA256 (pure Kotlin, portable across all targets)

internal fun hmacSha256(key: ByteArray, message: ByteArray): ByteArray {
    val blockSize = 64
    val normalizedKey =
        when {
            key.size > blockSize -> sha256(key).copyOf(blockSize)
            key.size < blockSize -> key.copyOf(blockSize)
            else -> key
        }

    val innerKey = ByteArray(blockSize) { index -> (normalizedKey[index].toInt() xor 0x36).toByte() }
    val outerKey = ByteArray(blockSize) { index -> (normalizedKey[index].toInt() xor 0x5c).toByte() }

    val innerHash = sha256(innerKey + message)
    return sha256(outerKey + innerHash)
}

private val SHA256_K = intArrayOf(
    0x428a2f98, 0x71374491, -0x4a3f0431, -0x164a245b, 0x3956c25b, 0x59f111f1, -0x6dc07d5c, -0x54e3a12b,
    -0x27f85568, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, -0x7f214e02, -0x6423f959, -0x3e640e8c,
    -0x1b64963f, -0x1041b87a, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    -0x67c1aeae, -0x57ce3993, -0x4ffcd838, -0x40a68039, -0x391ff40d, -0x2a586eb9, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, -0x7e3d36d2, -0x6d8dd37b,
    -0x5d40175f, -0x57e599b5, -0x3db47490, -0x3893ae5d, -0x2e6d17e7, -0x2966f9dc, -0xbf1ca7b, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, -0x7b3787ec, -0x7338fdf8, -0x6f410006, -0x5baf9315, -0x41065c09, -0x398e870e,
)

internal fun sha256(input: ByteArray): ByteArray {
    val h = intArrayOf(
        0x6a09e667, -0x4498517b, 0x3c6ef372, -0x5ab00ac6, 0x510e527f, -0x64fa9774, 0x1f83d9ab, 0x5be0cd19,
    )

    val messageLength = input.size
    val bitLength = messageLength.toLong() * 8

    // Pad: append 0x80, then zeros, then the 64-bit big-endian length.
    var paddedLength = messageLength + 1
    while (paddedLength % 64 != 56) paddedLength++
    val padded = ByteArray(paddedLength + 8)
    input.copyInto(padded)
    padded[messageLength] = 0x80.toByte()
    for (i in 0 until 8) {
        padded[paddedLength + i] = (bitLength ushr (56 - 8 * i)).toByte()
    }

    val w = IntArray(64)
    var offset = 0
    while (offset < padded.size) {
        for (i in 0 until 16) {
            val base = offset + i * 4
            w[i] = ((padded[base].toInt() and 0xFF) shl 24) or
                ((padded[base + 1].toInt() and 0xFF) shl 16) or
                ((padded[base + 2].toInt() and 0xFF) shl 8) or
                (padded[base + 3].toInt() and 0xFF)
        }
        for (i in 16 until 64) {
            val s0 = w[i - 15].rotateRight(7) xor w[i - 15].rotateRight(18) xor (w[i - 15] ushr 3)
            val s1 = w[i - 2].rotateRight(17) xor w[i - 2].rotateRight(19) xor (w[i - 2] ushr 10)
            w[i] = w[i - 16] + s0 + w[i - 7] + s1
        }

        var a = h[0]
        var b = h[1]
        var c = h[2]
        var d = h[3]
        var e = h[4]
        var f = h[5]
        var g = h[6]
        var hh = h[7]

        for (i in 0 until 64) {
            val s1 = e.rotateRight(6) xor e.rotateRight(11) xor e.rotateRight(25)
            val ch = (e and f) xor (e.inv() and g)
            val temp1 = hh + s1 + ch + SHA256_K[i] + w[i]
            val s0 = a.rotateRight(2) xor a.rotateRight(13) xor a.rotateRight(22)
            val maj = (a and b) xor (a and c) xor (b and c)
            val temp2 = s0 + maj

            hh = g
            g = f
            f = e
            e = d + temp1
            d = c
            c = b
            b = a
            a = temp1 + temp2
        }

        h[0] += a
        h[1] += b
        h[2] += c
        h[3] += d
        h[4] += e
        h[5] += f
        h[6] += g
        h[7] += hh

        offset += 64
    }

    val digest = ByteArray(32)
    for (i in 0 until 8) {
        digest[i * 4] = (h[i] ushr 24).toByte()
        digest[i * 4 + 1] = (h[i] ushr 16).toByte()
        digest[i * 4 + 2] = (h[i] ushr 8).toByte()
        digest[i * 4 + 3] = h[i].toByte()
    }
    return digest
}

private fun Int.rotateRight(bits: Int): Int = (this ushr bits) or (this shl (32 - bits))

// endregion
